                    ...
                    <div class="payment-history-row" data-payment-id="${p.id || ''}" data-amount="${p.amount}" data-date="${p.date || ''}" data-notes="${p.notes || ''}" style="background: white; padding: 14px 16px; border-left: 4px solid #667eea; border-radius: 10px; display: flex; justify-content: space-between; align-items: center; gap: 16px; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.05); transition: all 0.2s ease;">
                        ...
                        ${p.id ? `
                            <div style="display: flex; gap: 6px;">
                                <button type="button" class="btn-small btn-success payment-transfer-btn" data-payment-id="${p.id}" data-student-id="${document.getElementById('payment_student_id')?.value || ''}" data-amount="${p.amount}" title="Перевести сумму" style="border-radius: 8px;">↔️</button>
...
// Обработчик перевода оплаты
document.addEventListener('click', async (e) => {
    const btn = e.target.closest('.payment-transfer-btn');
    if (!btn) return;
    ...
    if (!paymentId || !studentId || !amount) {
        alert('Ошибка: не удалось получить данные об оплате');
        return;
    }
    
    if (typeof window.openTransferPaymentModal === 'function') {
        window.openTransferPaymentModal(paymentId, studentId, amount);
    } else {
        alert('Ошибка: функция перевода не загружена. Перезагрузите страницу.');
    }
});

// Открыть модальное окно перевода (глобальная функция)
window.openTransferPaymentModal = async function(paymentId, fromStudentId, maxAmount) {
    try {
        // Загрузить данные ученика-отправителя
        const studentResponse = await fetch(`/api/students/${fromStudentId}`);
        const studentData = await studentResponse.json();
        
        // Проверяем успешность ответа (может быть без поля success)
        if (studentData.error || (!studentData.full_name && !studentData.id)) {
            alert('Ошибка: не удалось загрузить данные ученика');
            return;
        }
        
        // Заполнить левую часть (отправитель)
        document.getElementById('transfer_payment_id').value = paymentId;
        document.getElementById('transfer_from_student_id').value = fromStudentId;
        document.getElementById('transfer_from_name').textContent = studentData.full_name || '—';
        document.getElementById('transfer_from_group').textContent = studentData.group_name || '—';
        document.getElementById('transfer_from_amount').textContent = maxAmount.toLocaleString('ru-RU', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + ' сум';
        document.getElementById('transfer_max_amount').textContent = maxAmount.toLocaleString('ru-RU', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
        document.getElementById('transfer_amount').max = maxAmount;
        
        // Загрузить группы для выбора
        const groupsResponse = await fetch('/api/groups');
        const groups = await groupsResponse.json();
        
        const groupSelect = document.getElementById('transfer_to_group');
        ...
        document.getElementById('transfer_to_student').innerHTML = '<option value="">Сначала выберите группу</option>';
        document.getElementById('transfer_to_student').disabled = true;
        document.getElementById('transfer_amount').value = '';
        document.getElementById('transfer_notes').value = '';
        
        // Показать модальное окно
        document.getElementById('transferPaymentModal').style.display = 'flex';
    } catch (error) {
        console.error('Ошибка открытия модального окна перевода:', error);
        alert('Ошибка: ' + error.message);
    }
}

// Закрыть модальное окно перевода (глобальная функция)
window.closeTransferPaymentModal = function() {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (groupSelect) {
        groupSelect.addEventListener('change', async function() {
            ...
            const studentSelect = document.getElementById('transfer_to_student');
            ...
            try {
                ...
                alert('Ошибка загрузки учеников группы');
            }
        });
    }
    
    // Обработчик отправки формы перевода
    const transferForm = document.getElementById('transferPaymentForm');
    if (transferForm) {
        transferForm.addEventListener('submit', async function(e) {
            e.preventDefault();
            
            const paymentId = document.getElementById('transfer_payment_id').value;
            const fromStudentId = document.getElementById('transfer_from_student_id').value;
            const toStudentId = document.getElementById('transfer_to_student').value;
            ...
            const maxAmountText = document.getElementById('transfer_max_amount').textContent;
            const maxAmount = parseFloat(maxAmountText.replace(/\s/g, '').replace('сум', '').replace(/,/g, '.'));
            
            if (!paymentId || !fromStudentId || !toStudentId || !amount || amount <= 0) {
                alert('Заполните все обязательные поля');
                return;
            }
            
            if (amount > maxAmount) {
                alert(`Сумма перевода не может превышать ${maxAmount.toLocaleString('ru-RU', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} сум`);
                return;
            }
            
            if (fromStudentId === toStudentId) {
                alert('Нельзя перевести средства самому себе');
                return;
            }
            
            if (!confirm(`Перевести ${amount.toLocaleString('ru-RU', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} сум выбранному ученику?`)) {
                return;
            }
            ...
            try {
                const response = await fetch('/api/payments/transfer', {
                    ...
                    body: JSON.stringify({
                        ...
                        from_student_id: parseInt(fromStudentId),
                        to_student_id: parseInt(toStudentId),
                        amount: amount,
                        notes: notes
                    })
                });
                ...
                if (result.success) {
                    alert('✓ Сумма успешно переведена!');
                    closeTransferPaymentModal();
                    location.reload(); // Перезагружаем страницу для обновления данных
                } else {
                    alert('Ошибка: ' + (result.message || 'Не удалось перевести сумму'));
                }
            } catch (error) {
