//three-dxf v0.1.0
console.log('three-dxf.js loading...');
...
(function(ThreeDxf) {
    ...
    ThreeDxf.Viewer = function(data, parent, width, height) {
        ...
        var dxfGroup = new THREE.Group();
        scene.add(dxfGroup);

        // Добавляем сетку как в AutoCAD
        var grid = new THREE.GridHelper(20000, 200, 0xdddddd, 0xf5f5f5);
        ...
        controls.enableRotate = false; // lock top-down view
        controls.screenSpacePanning = true;
        controls.mouseButtons = { LEFT: THREE.MOUSE.PAN, MIDDLE: THREE.MOUSE.DOLLY, RIGHT: THREE.MOUSE.PAN };
        this.controls = controls; // expose for consumers
        this.scene = scene;
        this.camera = camera;
        ...
        this.resetView = function() {
            ...
            // Центрируем сетку по чертежу
            grid.position.set(center.x, center.y, -0.1);
            var maxDim = Math.max(size.x, size.y);
            if (maxDim > 10000) {
                var s = (maxDim / 10000) * 2;
            ...
            var margin = 1.1;
            var halfW = (size.x * margin) / 2;
            var halfH = (size.y * margin) / 2;
            var currentAspect = (renderer.domElement.clientWidth || width) / (renderer.domElement.clientHeight || height);
            if (currentAspect > 0) {
                if (halfW / halfH > currentAspect) {
                    halfH = halfW / currentAspect;
                } else {
                    halfW = halfH * currentAspect;
                }
            }
