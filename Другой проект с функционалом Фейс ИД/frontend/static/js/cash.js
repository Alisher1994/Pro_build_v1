...
async function loadCashBalance() {
    try {
        ...
        if (balanceInput) {
            ...
            // Показываем отрицательный баланс красным цветом
            const formattedBalance = currentBalance.toLocaleString('ru-RU', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
            balanceInput.value = formattedBalance + ' сум';
            if (currentBalance < 0) {
        ...
        // НЕ подставляем сумму автоматически - пользователь должен ввести вручную
        // Это предотвращает случайную передачу отрицательной суммы
...
function openAddTransferModal() {
    ...
    if (modal && title && form && editId) {
        ...
        // Загрузить баланс и подставить в поле суммы (только если баланс положительный)
        loadCashBalance().then(() => {
            const amountInput = document.getElementById('transfer-amount');
            if (amountInput) {
                // Подставляем только положительный баланс, иначе оставляем пустым
                if (currentBalance > 0) {
