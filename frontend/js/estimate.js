...
const EstimateManager = {
    ...
    currentRightPanelTab: '3d', // '3d' или 'files'

    // Конфигурация статусов смет
    statusConfig: {
        draft: { text: 'Черновик', colorClass: 'status-draft', svg: '<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/>' },
        pending: { text: 'На утверждении', colorClass: 'status-pending', svg: '<circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>' },
        approved: { text: 'Утвержден', colorClass: 'status-approved', svg: '<path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/>' },
        cancelled: { text: 'Отменен', colorClass: 'status-cancelled', svg: '<circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/>' }
    },
    ...
    renderStatusDropdown(estimateId, currentStatus) {
        ...
        for (const [key, val] of Object.entries(this.statusConfig)) {
            optionsHtml += `
                <div class="status-dropdown-option ${key === currentStatus ? 'selected' : ''}" 
                     onclick="EstimateManager.selectStatus('${estimateId}', '${key}', event)">
                    <span class="status-icon ${val.colorClass}">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            ${val.svg}
                        </svg>
                    </span>
                    <span class="status-text">${val.text}</span>
                </div>
            `;
        }

        return `
            <div class="status-dropdown" id="status-dropdown-${estimateId}">
                <div class="status-dropdown-selected" onclick="EstimateManager.toggleStatusDropdown('${estimateId}', event)">
                    <span class="status-icon ${config.colorClass}">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            ${config.svg}
                        </svg>
                    </span>
    ...
    async selectStatus(estimateId, newStatus, event) {
        ...
        await this.updateEstimateStatus(estimateId, newStatus);
    },

    // Фильтры дерева сметы
    treeFilters: {
        workTypes: false,
        labor: false,
        equipment: false,
    ...
    async exportToCSV() {
        if (!this.currentEstimateId) {
            UI.showNotification('Смета не выбрана', 'warning');
            return;
        }
        ...
        try {
            ...
            let csvContent = 'N п.п.,Шифр номера нормативов и коды ресурсов,Наименование работ и затрат,Единица измерения,Количество,,Цена,Сумма\n';
            csvContent += ',,,,на. ед. измерения,по проектным данным,,\n';
            csvContent += '1,2,3,4,5,6,7,8\n';

            estimate.sections.forEach((section, sIdx) => {
                // Пропускаем вывод названия секции, если это стандартный контейнер "Раздел сметы"
                // или если мы хотим максимально соответствовать исходному формату (где только этапы)
                if (section.name && section.name !== 'Раздел сметы' && section.name !== 'Базовый раздел') {
                    csvContent += `"${section.name}",,,,,\n`;
                }

                section.stages.forEach(stage => {
                    // Добавляем строку этапа (например, РАЗДЕЛ 1. КРОВЛЯ)
                    // Пропускаем технические названия этапов
                    if (stage.name && stage.name !== 'Без названия' && stage.name !== 'Основной этап') {
                        csvContent += `"${stage.name}",,,,,\n`;
                    }

                    // В текущей структуре БД Stage - это группа, а WorkType - это строка работы
                    stage.workTypes.forEach(work => {
                        // Пропускаем технические строки, которые могли попасть в БД при старом импорте
                        if (work.code === '1' && work.cipher === '2' && work.name === '3') {
                            return;
                        }

                        // Вид работ
                        const workCode = work.code || '';
                        const workCipher = work.cipher || '';
                        const workName = (work.name || '').replace(/"/g, '""');
                        const workUnit = work.unit || '';
                        const workQtyNum = Number(work.quantity) || 0;

                        // Стоимость по виду работ: берём сохранённую, иначе считаем по ресурсам
                        let workTotal = Number(work.totalCost);
                        if (!workTotal || Number.isNaN(workTotal)) {
                            workTotal = (work.resources || []).reduce((sum, r) => {
                                const rPrice = Number(r.unitPrice ?? r.price ?? 0) || 0;
                                const rTotal = Number(r.totalCost);
                                const calc = (!Number.isNaN(rTotal) && rTotal !== null && rTotal !== undefined) ? rTotal : (Number(r.quantity) || 0) * rPrice;
                                return sum + calc;
                            }, 0);
                        }

                        const workPrice = workQtyNum > 0 ? workTotal / workQtyNum : workTotal;

                        csvContent += `"${workCode}","${workCipher}","${workName}","${workUnit}","","${fmtQty(workQtyNum)}","${fmtMoney(workPrice)}","${fmtMoney(workTotal)}"\n`;

                        // Ресурсы
                        work.resources.forEach(res => {
                            const resCode = res.code || '';
                            const resCipher = res.cipher || '';
                            const resName = (res.name || '').replace(/"/g, '""');
                            const resUnit = res.unit || '';
                            const resNorm = res.normPerUnit;
                            const resQtyNum = Number(res.quantity) || 0;
                            const resUnitPrice = Number(res.unitPrice ?? res.price ?? 0) || 0;
                            const resTotal = (() => {
                                const t = Number(res.totalCost);
                                return (!Number.isNaN(t) && t !== null && t !== undefined) ? t : resQtyNum * resUnitPrice;
                            })();

                            csvContent += `"${resCode}","${resCipher}","${resName}","${resUnit}","${fmtQty(resNorm)}","${fmtQty(resQtyNum)}","${fmtMoney(resUnitPrice)}","${fmtMoney(resTotal)}"\n`;
                        });
                    });
                });
            });
            ...
            UI.showNotification('Файл успешно экспортирован', 'success');
        } catch (error) {
    ...
    async toggleTreeFilter(filterKey) {
        ...
        if (toggle) {
            toggle.style.background = this.treeFilters[filterKey] ? 'var(--primary)' : 'var(--gray-300)';
            const dot = toggle.querySelector('div');
            if (dot) dot.style.left = this.treeFilters[filterKey] ? '16px' : '2px';
        }

        // Обновляем стиль основной кнопки фильтра (только если панель закрыта)
        const filterBtn = document.getElementById('tree-filter-btn');
        const container = document.getElementById('tree-filter-toggles');
        const isAnyActive = Object.values(this.treeFilters).some(v => v);

        if (filterBtn && container && container.style.display !== 'flex') {
            filterBtn.style.background = isAnyActive ? 'var(--primary-lighter)' : 'var(--white)';
            filterBtn.style.borderColor = isAnyActive ? 'var(--primary)' : 'var(--gray-300)';
            filterBtn.style.color = isAnyActive ? 'var(--primary)' : 'var(--gray-700)';
        }

        // Если включен любой фильтр, раскрываем дерево до нужного уровня
        if (isAnyActive) {
            ...
            if (resourceFiltersActive) {
                ...
                // Если активен только фильтр "Виды работ", раскрываем только до работ
                await this.expandToWorkTypes();
            }
        } else {
            // Если все фильтры выключены, сворачиваем дерево обратно и сбрасываем состояние
    ...
    // Обновить только видимые части дерева с учетом фильтров
    ...
    // Показать/скрыть панель фильтров (теперь в строку)
    toggleFilterPanel(event) {
        event.stopPropagation();
        const container = document.getElementById('tree-filter-toggles');
        const btn = document.getElementById('tree-filter-btn');
        ...
        if (isVisible) {
            ...
            const isAnyActive = Object.values(this.treeFilters).some(v => v);
            btn.style.background = isAnyActive ? 'var(--primary-lighter)' : 'var(--white)';
            btn.style.borderColor = isAnyActive ? 'var(--primary)' : 'var(--gray-300)';
            btn.style.color = isAnyActive ? 'var(--primary)' : 'var(--gray-700)';
        } else {
            // Генерируем переключатели, если их еще нет или нужно обновить
            this.renderFilterToggles(container);
            container.style.display = 'flex';
            // Заливаем кнопку при открытии
    ...
    renderFilterToggles(container) {
        const filters = [
            { key: 'workTypes', label: 'Виды работ' },
            { key: 'labor', label: 'Труд' },
            { key: 'equipment', label: 'Техника' },
            { key: 'material', label: 'Мат-лы' }
        ];

        let html = '';
        filters.forEach((f, index) => {
            const isActive = this.treeFilters[f.key];
            html += `
                <div style="display: flex; align-items: center; gap: 6px; cursor: pointer; user-select: none; opacity: 0; transform: translateX(-10px); animation: slideInFilter 0.2s ease forwards ${index * 0.05}s;" onclick="EstimateManager.toggleTreeFilter('${f.key}')">
                    <span style="font-size: 11px; font-weight: 600; color: var(--gray-600); white-space: nowrap;">${f.label}</span>
                    <div id="filter-toggle-${f.key}" class="filter-switch ${isActive ? 'active' : ''}" style="
                        width: 30px; 
                        height: 16px; 
                        background: ${isActive ? 'var(--primary)' : 'var(--gray-300)'}; 
                        border-radius: 8px; 
                        position: relative; 
                        transition: all 0.2s;
                    ">
    ...
    async updateBreadcrumb(items = null) {
        ...
        // Кэшируем для восстановления после перезагрузки/перерисовки
        this.setBreadcrumbCache(items);

        if (window.app && typeof window.app.updateBreadcrumbs === 'function') {
            window.app.updateBreadcrumbs(items);
        }

        this.updateStructureTitle(items);
    },

    setBreadcrumbCache(items = []) {
        this.cachedBreadcrumbItems = items;
        try {
            const safeItems = items.map(it => ({ text: it.text || '' }));
            localStorage.setItem('probim_breadcrumb_cache', JSON.stringify(safeItems));
        } catch (e) {
            console.warn('Could not cache breadcrumbs', e);
        }
    },

    loadBreadcrumbCache() {
        if (this.cachedBreadcrumbItems && this.cachedBreadcrumbItems.length) return this.cachedBreadcrumbItems;
        try {
            const raw = localStorage.getItem('probim_breadcrumb_cache');
            if (!raw) return [];
            const parsed = JSON.parse(raw);
            if (Array.isArray(parsed)) {
                this.cachedBreadcrumbItems = parsed;
                return parsed;
            }
        } catch (e) {
            console.warn('Could not load cached breadcrumbs', e);
        }
        return [];
    },

    getActiveBreadcrumbName(items = null) {
        const list = items && items.length ? items : this.loadBreadcrumbCache();
        return list && list.length ? list[list.length - 1].text : '';
    },

    updateStructureTitle(items = null) {
        const titleEl = document.getElementById('estimate-structure-active');
        if (!titleEl) return;
        const name = this.getActiveBreadcrumbName(items);
        ...
        return `
            <div class="tree-item block-item" data-block-id="${block.id}" onclick="EstimateManager.openBlock('${block.id}')" style="cursor: pointer;">
                <div class="tree-item-header">
                    ...
                    <button class="btn" onclick="event.stopPropagation(); EstimateManager.deleteBlock('${block.id}')" style="width: 32px; height: 32px; padding: 0; display: flex; align-items: center; justify-content: center; margin-right: 8px; background: var(--white); border: 1px solid var(--danger); color: var(--danger);" onmouseover="this.style.background='var(--gray-50)'" onmouseout="this.style.background='var(--white)'" title="Удалить">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    async loadEstimates(blockId) {
        try {
            ...
            html += '<th>Название сметы</th><th>Описание</th><th>Стоимость</th><th>Статус</th><th style="width: 28%;">IFC модель</th><th>Действия</th>';
            html += '</tr></thead><tbody>';
                ...
                html += `
                    <tr style="cursor: pointer;" onclick="EstimateManager.openEstimate('${estimateId}')">
                        ...
                        <td onclick="event.stopPropagation()">
                            ${this.renderStatusDropdown(estimateId, status)}
                        </td>
                        <td>
                        ...
                        <td>
                            ...
                            <button onclick="event.stopPropagation(); EstimateManager.deleteEstimate('${estimate.id}')" class="btn" style="width: 36px; height: 36px; padding: 0; display: inline-flex; align-items: center; justify-content: center; background: var(--white); border: 1px solid var(--danger); color: var(--danger);" onmouseover="this.style.background='var(--gray-50)'" onmouseout="this.style.background='var(--white)'" title="Удалить" aria-label="Удалить">
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                    <polyline points="3 6 5 6 21 6"/>
                                    <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/>
    ...
    async editEstimate(estimateId) {
        try {
            ...
            const content = `
                ...
                <div class="form-group">
                    <label>Статус</label>
                    <select id="estimate-status" class="form-control">
                        <option value="draft" ${estimate.status === 'draft' ? 'selected' : ''}>Черновик</option>
                        <option value="pending" ${estimate.status === 'pending' ? 'selected' : ''}>На утверждении</option>
                        <option value="approved" ${estimate.status === 'approved' ? 'selected' : ''}>Утвержден</option>
                        <option value="cancelled" ${estimate.status === 'cancelled' ? 'selected' : ''}>Отменен</option>
                    </select>
                </div>
            `;
    ...
    async updateEstimateStatus(estimateId, newStatus) {
        try {
            ...
            if (dropdownContainer) {
                dropdownContainer.innerHTML = this.renderStatusDropdown(estimateId, newStatus);
            }
        } catch (error) {
            UI.showNotification('Ошибка при обновлении статуса: ' + error.message, 'error');
            // Перезагружаем список, чтобы вернуть старое значение
            this.loadEstimates(this.currentBlockId);
        }
    },
                ...
                <div style="height: 100%; display: flex; flex-direction: column;">
                    ...
                    <div style="flex: 1; display: flex; overflow: hidden;">
                        ...
                        <div id="left-panel" style="width: 60%; background: var(--white); border-right: 1px solid var(--gray-300); display: flex; flex-direction: column; overflow: hidden;">
                            <div style="height: 56px; padding: 0 16px; background: var(--gray-50); border-bottom: 1px solid var(--gray-300); display: flex; justify-content: space-between; align-items: center; box-sizing: border-box; flex-shrink: 0;">
                                <div style="display: flex; align-items: center; gap: 8px;">
                                    <h3 style="margin: 0; font-size: 14px; font-weight: 600; color: var(--gray-700); display: flex; align-items: center; gap: 6px;">
                                        <span>Структура сметы</span>
                                        <span id="estimate-structure-active" style="font-weight: 700; color: var(--gray-800);"></span>
                                    </h3>
                                    <div style="display: flex; align-items: center; gap: 4px;">
                                        <button id="tree-filter-btn" onclick="EstimateManager.toggleFilterPanel(event)" class="btn btn-sm" title="Фильтры" style="width: 32px; height: 32px; padding: 0; display: flex; align-items: center; justify-content: center; flex-shrink: 0; ${Object.values(this.treeFilters).some(v => v) ? 'background: var(--primary-lighter); border: 1px solid var(--primary); color: var(--primary);' : 'background: var(--white); border: 1px solid var(--gray-300); color: var(--gray-700);'}">
                                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                                <polygon points="22 3 2 3 10 12.46 10 19 14 21 14 12.46 22 3"></polygon>
                                            </svg>
                                        </button>
                                        <div id="tree-filter-toggles" style="display: none; align-items: center; gap: 12px; margin-left: 4px; padding: 0 8px; border-left: 1px solid var(--gray-300); overflow: hidden; transition: all 0.3s ease;">
                        ...
                        <div id="resize-divider" style="width: 4px; background: var(--gray-100); cursor: col-resize; flex-shrink: 0;"></div>

                        <!-- Правая часть: 3D модель и свойства -->
                        <div class="right-panel-container" id="right-panel-container">
                            <!-- Табы в свернутом состоянии -->
                            <div class="right-panel-tabs-collapsed">
                                <button class="right-panel-toggle-collapsed" onclick="EstimateManager.toggleRightPanel(event)" title="Развернуть панель" style="margin-bottom: 12px; background: white; border: 1px solid var(--gray-300); border-radius: 4px; width: 32px; height: 32px; display: flex; align-items: center; justify-content: center; cursor: pointer; color: var(--gray-600);">
                                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                        <polyline points="15 18 9 12 15 6"></polyline>
                                    </svg>
                                </button>
                                <div class="right-panel-tab-collapsed ${this.currentRightPanelTab === '3d' ? 'active' : ''}" 
                                     data-tab="3d" onclick="EstimateManager.switchRightPanelTab('3d')">
                                    3D Вид и Свойства
                                </div>
                                <div class="right-panel-tab-collapsed ${this.currentRightPanelTab === 'files' ? 'active' : ''}" 
                                     data-tab="files" onclick="EstimateManager.switchRightPanelTab('files')">
                                    Файлы проекта
                                </div>
                            </div>

                            <div class="right-panel-content">
                                <!-- Табы в развернутом состоянии -->
                                <div class="right-panel-tabs-expanded" style="display: flex; align-items: stretch; justify-content: space-between; width: 100%; box-sizing: border-box;">
                                    <div style="display: flex; align-items: stretch;">
                                        <div class="right-panel-tab-expanded ${this.currentRightPanelTab === '3d' ? 'active' : ''}" 
                                             data-tab="3d" onclick="EstimateManager.switchRightPanelTab('3d')">
                                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                        ...
                                        <div class="right-panel-tab-expanded ${this.currentRightPanelTab === 'files' ? 'active' : ''}" 
                                             data-tab="files" onclick="EstimateManager.switchRightPanelTab('files')">
                                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                    ...
                                    <button class="right-panel-toggle" id="right-panel-toggle" title="Свернуть панель" style="position: static; margin-right: 8px; flex-shrink: 0; align-self: center;">
                                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                ...
                                <!-- Контент: 3D Вид -->
                                <div id="right-panel-content-3d" style="flex: 1; display: ${this.currentRightPanelTab === '3d' ? 'flex' : 'none'}; flex-direction: column; overflow: hidden;">
                                    <!-- 3D Viewer -->
                                ...
                                <!-- Контент: Файлы -->
                                <div id="right-panel-content-files" style="flex: 1; display: ${this.currentRightPanelTab === 'files' ? 'flex' : 'none'}; flex-direction: column; overflow: hidden; background: var(--white);">
                                    <div id="estimate-files-container" style="flex: 1; display: flex; flex-direction: column; overflow: hidden;">
                                        <!-- File list or viewer will be here -->
    ...
    initRightPanelToggle() {
        ...
        if (savedState === 'true') {
            ...
            const leftPanel = document.getElementById('left-panel');
            if (leftPanel) {
                leftPanel.style.width = 'calc(100% - 48px)';
            }
        }

        if (toggleBtn) {
            toggleBtn.onclick = (e) => this.toggleRightPanel(e);
        }
    },
    ...
    toggleRightPanel(e) {
        ...
        if (leftPanel) {
            if (isCollapsed) {
                ...
                // Сохраняем только если это не calc (чтобы не перезаписать нормальную ширину)
                if (currentWidth && !currentWidth.includes('calc')) {
                    localStorage.setItem('leftPanelWidthBeforeCollapse', currentWidth);
                }
                leftPanel.style.width = 'calc(100% - 48px)';
            } else {
                let savedWidth = localStorage.getItem('leftPanelWidthBeforeCollapse') || localStorage.getItem('leftPanelWidth') || '60%';
                if (typeof savedWidth === 'string' && !savedWidth.includes('%') && !savedWidth.includes('calc')) {
                    savedWidth += '%';
                }
                leftPanel.style.width = savedWidth;
        ...
        // Обновляем размер viewer если он есть
    ...
    updateRightPanelContent() {
        const content3d = document.getElementById('right-panel-content-3d');
        const contentFiles = document.getElementById('right-panel-content-files');

        // Обновляем активные классы табов (expanded)
        document.querySelectorAll('.right-panel-tab-expanded').forEach(t => {
            t.classList.toggle('active', t.dataset.tab === this.currentRightPanelTab);
        });

        // Обновляем активные классы табов (collapsed)
        document.querySelectorAll('.right-panel-tab-collapsed').forEach(t => {
            t.classList.toggle('active', t.dataset.tab === this.currentRightPanelTab);
        });

        if (this.currentRightPanelTab === '3d') {
            if (content3d) content3d.style.display = 'flex';
            if (contentFiles) contentFiles.style.display = 'none';
        } else {
            if (content3d) content3d.style.display = 'none';
            if (contentFiles) contentFiles.style.display = 'flex';
            this.loadEstimateFiles();
        }
    },

    async loadEstimateFiles() {
        const container = document.getElementById('estimate-files-container');
        if (!container) return;

        if (!this.currentEstimateId) {
            container.innerHTML = '<div style="padding: 20px; text-align: center; color: var(--gray-500);">Смета не выбрана</div>';
            return;
        }

        // Сохраняем список файлов для быстрого доступа
        this._estimateFiles = [];

        container.innerHTML = `
            <div class="files-header-bar" id="files-header-bar" style="padding: 16px; border-bottom: 1px solid var(--gray-200); display: flex; justify-content: space-between; align-items: center; background: var(--gray-50);">
                <h3 style="margin: 0; font-size: 14px; font-weight: 600; color: var(--gray-700);">Файлы проекта</h3>
                <button class="btn btn-primary btn-sm" onclick="EstimateManager.uploadProjectFile()">
            ...
            <div id="files-content-area" style="flex: 1; display: flex; flex-direction: column; overflow: hidden;">
                <div id="files-list" style="flex: 1; overflow-y: auto; padding: 8px;">
        ...
        try {
            const files = await api.getEstimateFiles(this.currentEstimateId);
            this._estimateFiles = files || [];
            this.renderFilesList();
        } catch (error) {
            console.error('Error loading files:', error);
            const listContainer = document.getElementById('files-list');
            if (listContainer) {
                listContainer.innerHTML = `<div style="color: var(--red-500); padding: 20px;">Ошибка загрузки: ${error.message}</div>`;
            }
        }
    },
    ...
    renderFilesList() {
        ...
        // Очищаем область и создаем контейнер для списка, если его нет
        contentArea.innerHTML = `
            <div id="files-list" style="flex: 1; overflow-y: auto; padding: 8px;"></div>
        `;
        ...
        listContainer.innerHTML = files.map(file => {
            ...
            if (fileExt === 'pdf') {
                ...
                iconHtml = `<img src="img/dwg.png" style="width: 32px; height: 32px; object-fit: contain;" alt="DWG">`;
            } else {
                iconHtml = `<div style="width: 32px; height: 32px; display: flex; align-items: center; justify-content: center; background: var(--gray-100); color: var(--gray-500); border-radius: 4px; font-size: 10px; font-weight: bold;">${fileExt.toUpperCase()}</div>`;
            }

            return `
                <div class="file-item" style="display: flex; align-items: center; padding: 10px; border-radius: 6px; margin-bottom: 4px; transition: background 0.2s; border: 1px solid transparent; cursor: pointer;" 
                     onmouseover="this.style.background='var(--gray-50)'; this.style.borderColor='var(--gray-200)'" 
                     onmouseout="this.style.background='transparent'; this.style.borderColor='transparent'"
                     onclick="EstimateManager.viewProjectFile('${file.id}')">
                    ...
                    <div style="flex: 1; min-width: 0;">
                        <div style="font-size: 13px; font-weight: 500; color: var(--gray-700); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" title="${file.name}">
                            ${file.name}
                        </div>
                    ...
                    <div style="display: flex; gap: 4px;" onclick="event.stopPropagation()">
                        <button onclick="EstimateManager.deleteProjectFile('${file.id}')" class="btn btn-sm" title="Удалить" style="width: 26px; height: 26px; padding: 0; border-radius: 4px; display: inline-flex; align-items: center; justify-content: center; background: var(--white); border: 1px solid var(--danger); color: var(--danger);" onmouseover="this.style.background='var(--gray-50)'" onmouseout="this.style.background='var(--white)'">×</button>
                    </div>
                </div>
            `;
        }).join('');
    ...
    viewProjectFile(fileId) {
        const file = this._estimateFiles.find(f => f.id === fileId);
        if (!file) return;
        ...
        // Формируем URL файла
        const fileUrl = file.fileUrl.startsWith('uploads/') ? '/' + file.fileUrl : file.fileUrl;
        const previewUrl = file.previewUrl ? (file.previewUrl.startsWith('uploads/') ? '/' + file.previewUrl : file.previewUrl) : null;

        const fileExt = file.name.split('.').pop().toLowerCase();
        let iconHtml = '';
        if (fileExt === 'pdf') {
            iconHtml = `<img src="img/pdf.png" style="width: 20px; height: 20px; object-fit: contain;" alt="PDF">`;
        } else if (fileExt === 'dwg' || fileExt === 'dxf') {
            iconHtml = `<img src="img/dwg.png" style="width: 20px; height: 20px; object-fit: contain;" alt="DWG">`;
        }

        contentArea.innerHTML = `
            <div style="display: flex; flex-direction: column; height: 100%; background: #f8fafc;">
                <div style="padding: 8px 16px; background: white; border-bottom: 1px solid var(--gray-200); display: flex; align-items: center; gap: 12px;">
                    <button class="btn btn-sm" onclick="EstimateManager.renderFilesList()" style="padding: 4px; display: flex; align-items: center; gap: 4px; color: var(--gray-600);">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    ...
                    <div style="display: flex; align-items: center; gap: 8px; flex: 1; min-width: 0;">
                        ${iconHtml}
                        <div style="font-size: 13px; font-weight: 600; color: var(--gray-800); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                            ${file.name}
                        </div>
                    </div>
                    <a href="${fileUrl}" download="${file.name}" class="btn btn-secondary btn-sm" style="width: 28px; height: 28px; display: flex; align-items: center; justify-content: center; padding: 0; border-radius: 4px;" title="Скачать">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
                    ...
                    ${file.fileType === 'dxf' ? `
                        <button class="btn btn-secondary btn-sm" onclick="EstimateManager.centerDxfView()" style="height: 28px; display: flex; align-items: center; gap: 6px; border-radius: 4px;">
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                ...
                <div style="flex: 1; position: relative; overflow: hidden; display: flex; align-items: stretch; justify-content: stretch;">
                    ${(file.fileType === 'pdf' || previewUrl) ? `
                        <iframe src="${(previewUrl || fileUrl)}#toolbar=0" style="width: 100%; height: 100%; border: none;"></iframe>
                    ` : file.fileType === 'dxf' ? `
                        <div id="dxf-viewer-container" style="width: 100%; height: 100%; background: #222;"></div>
                    ` : `
                        <div style="text-align: center; padding: 40px; color: var(--gray-500);">
                            <div style="width: 80px; height: 80px; background: #e0f2fe; color: #0ea5e9; border-radius: 12px; display: flex; align-items: center; justify-content: center; margin: 0 auto 16px;">
                                <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                    <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path>
                                    <polyline points="14 2 14 8 20 8"></polyline>
                                </svg>
                            </div>
                            <h4 style="margin: 0 0 8px; color: var(--gray-800);">Превью для этого файла недоступно</h4>
                            <p style="font-size: 13px; margin-bottom: 20px;">Для просмотра DWG требуется установленный QCAD на сервере.</p>
                            <a href="${fileUrl}" download="${file.name}" class="btn btn-secondary">
                                Скачать файл
                            </a>
                        </div>
                    `}
        ...
        // После рендера подставляем активное имя в заголовок
        this.updateStructureTitle();

        // Если это DXF, инициализируем вьювер после рендера контейнера
        if (file.fileType === 'dxf') {
            setTimeout(() => this.initDxfViewer(fileUrl), 100);
        }
    },

    async initDxfViewer(fileUrl) {
        ...
        try {
            ...
            if (typeof DxfParser === 'undefined') {
                container.innerHTML = '<div style="color: white; padding: 20px;">Ошибка: DxfParser не загружен</div>';
                return;
            }

            container.innerHTML = '<div style="color: white; padding: 20px;">Загрузка и обработка чертежа...</div>';
            ...
            if (dxf) {
                ...
                const cadCanvas = new DxfViewerLib.Viewer(dxf, container, container.clientWidth, container.clientHeight);
                this.currentDxfViewer = cadCanvas;
                if (typeof cadCanvas.resetView === 'function') {
                    cadCanvas.resetView();
                }
                // Ensure light background and dark entities so white lines stay visible
                if (cadCanvas.renderer && cadCanvas.renderer.setClearColor) {
                    cadCanvas.renderer.setClearColor(0xf7f7f7, 1);
                }
                let recolored = 0;
                cadCanvas.scene?.traverse((obj) => {
                    const materials = Array.isArray(obj.material) ? obj.material : obj.material ? [obj.material] : [];
                    materials.forEach((m) => {
                        if (m && m.color && typeof m.color.getHex === 'function' && m.color.getHex() === 0xffffff) {
                ...
                const resizeHandler = () => {
                    if (container && container.clientWidth > 0) {
                        // ThreeDxf.Viewer не имеет встроенного resize, 
                        // но мы можем пересоздать его или обновить камеру если нужно
                    }
                };
            ...
            container.innerHTML = `<div style="color: #ef4444; padding: 20px;">Ошибка при чтении DXF: ${error.message}</div>`;
        }
    },

    centerDxfView() {
        if (this.currentDxfViewer && typeof this.currentDxfViewer.resetView === 'function') {
            this.currentDxfViewer.resetView();
        } else {
            UI.showNotification('Вьюер DXF ещё не инициализировался', 'warning');
        }
    },
    ...
    async deleteProjectFile(fileId) {
        UI.confirmDelete('Удалить этот файл?', async () => {
            try {
                await api.deleteEstimateFile(fileId);
                        ...
                        <div onclick="EstimateManager.toggleStage('${stage.id}')" style="padding: 8px 12px; background: #f0f9f5; border-left: 3px solid var(--primary); cursor: pointer; border-radius: 4px;">
                            <div style="display: flex; align-items: center; justify-content: space-between;">
                                ...
                                <div style="display: flex; align-items: center; gap: 8px;">
                                    ...
                                    <button onclick="EstimateManager.deleteStage('${stage.id}'); event.stopPropagation();" class="btn btn-sm" title="Удалить этап" style="width: 26px; height: 26px; padding: 0; border-radius: 4px; display: inline-flex; align-items: center; justify-content: center; background: var(--white); border: 1px solid var(--danger); color: var(--danger);" onmouseover="this.style.background='var(--gray-50)'" onmouseout="this.style.background='var(--white)'">×</button>
                                </div>
                            </div>
                        </div>
        ...
        let html = `
            <div class="resource-item" data-resource-id="${resource.id}" data-has-ifc-link="${hasIfcLink}" data-ifc-elements='${ifcElementsJson}' style="background: var(--white); border: 1px solid var(--gray-200); border-radius: 3px; padding: 6px 8px; margin-bottom: 3px; font-size: 11px; cursor: pointer; transition: all 0.15s ease;">
                <div style="display: flex; align-items: center; gap: 8px;">
                    ...
                    <span class="res-quantity-editable" data-res-id="${resource.id}" style="cursor: text; padding: 2px 4px; background: var(--gray-100); border-radius: 2px; min-width: 40px; text-align: right;">${(resource.quantity === null || resource.quantity === undefined) ? '' : resource.quantity}</span>
                    ×
            ...
            for (const section of sections) {
                ...
                const ifcButton = section.ifcFileUrl
                    ? `<div style="display: flex; gap: 4px;">
                        ...
                        <button onclick="EstimateManager.unlinkSectionIFC('${section.id}')" class="btn" style="padding: 4px 8px; height: auto; background: var(--white); border: 1px solid var(--danger); color: var(--danger);" onmouseover="this.style.background='var(--gray-50)'" onmouseout="this.style.background='var(--white)'" title="Отвязать IFC">
                ...
                html += `
                    <tr>
                        ...
                        <td>
                            ...
                            <button onclick="EstimateManager.deleteSection('${section.id}')" class="btn" style="padding: 4px 12px; height: auto; background: var(--white); border: 1px solid var(--danger); color: var(--danger);" onmouseover="this.style.background='var(--gray-50)'" onmouseout="this.style.background='var(--white)'">
                                Удалить
                            </button>
                        </td>
                    </tr>
    ...
    async loadWorkTypesTree(stageId) {
        try {
            ...
            // Применяем фильтры
            const isFilterActive = Object.values(this.treeFilters).some(v => v === true);
            let filteredWorkTypes = workTypes;

            if (isFilterActive) {
                const filtered = [];
                for (const wt of workTypes) {
                    // Если включен фильтр "Виды работ", показываем его
                    if (this.treeFilters.workTypes) {
                        filtered.push(wt);
                        continue;
                    }
                    ...
                    const hasMatchingResource = resources.some(r => {
                        ...
                        if (this.treeFilters.labor && type === 'labor') return true;
                        if (this.treeFilters.equipment && type === 'equipment') return true;
                ...
                filteredWorkTypes = filtered;
            }

            if (filteredWorkTypes.length === 0 && isFilterActive) {
                container.innerHTML = `<div style="padding: 8px; color: var(--gray-500); font-size: 13px;">Нет данных по выбранным фильтрам</div>`;
                return;
            }
            ...
            for (let index = 0; index < filteredWorkTypes.length; index++) {
                const workType = filteredWorkTypes[index];
                        ...
                        <div style="padding: 6px 10px; background: var(--white); border: 1px solid var(--gray-300); border-radius: 4px; font-size: 13px; cursor: pointer;" onclick="EstimateManager.selectWorkType('${workType.id}')">
                            <div style="display: flex; justify-content: space-between; align-items: center; gap: 8px;">
                                <div style="display: flex; align-items: center; gap: 8px; min-width: 0; flex: 1;">
                                    ${expander}
                                    <span style="min-width: 64px; text-align: center; font-size: 12px; color: var(--gray-500); flex-shrink: 0; padding: 0 4px;">${displayNo}.</span>
                                    <div style="display: flex; align-items: center; gap: 6px; min-width: 0; flex: 1;">
                                        <span class="worktype-cipher-inline" title="Шифр" style="width: 130px; flex-shrink: 0; padding: 1px 4px; background: ${workType.cipher ? 'var(--gray-100)' : 'transparent'}; border: 1px solid ${workType.cipher ? 'var(--gray-300)' : 'transparent'}; border-radius: 3px; color: var(--gray-700); font-size: 10px; font-weight: 600; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; display: inline-block; text-align: center; box-sizing: border-box;">${workType.cipher ? this.escapeHtml(workType.cipher) : ''}</span>
                                        ...
                                        ${hasResources ? `<span style="font-size: 11px; color: var(--gray-500); margin-left: 4px;">(${resourcesCount})</span>` : ''}
                                    </div>
                                    
                                    <div style="display: flex; align-items: center; gap: 4px; flex-shrink: 0; font-size: 12px; color: var(--gray-600); margin-left: auto; width: 460px; justify-content: flex-start;">
                                        <div style="width: 65px; flex-shrink: 0;"></div>
                                        <span class="worktype-unit-inline" data-worktype-id="${workType.id}" style="width: 56px; text-align: left; cursor: text; padding: 1px 4px; border-radius: 3px;" onmouseover="this.style.background='var(--gray-100)'" onmouseout="this.style.background='transparent'">${this.escapeHtml(workType.unit ?? '')}</span>
                                        <span class="worktype-quantity-inline" data-worktype-id="${workType.id}" style="width: 80px; text-align: right; cursor: text; padding: 1px 4px; border-radius: 3px;" onmouseover="this.style.background='var(--gray-100)'" onmouseout="this.style.background='transparent'">${(workType.quantity === null || workType.quantity === undefined) ? '' : workType.quantity}</span>
                                        <span style="width: 12px; text-align: center; color: var(--gray-400);">×</span>
                                        <span class="worktype-unitcost-inline" data-worktype-id="${workType.id}" style="width: 70px; text-align: right; padding: 1px 4px; border-radius: 3px;">${UI.formatNumber(wtUnitCostAuto, 2)}</span>
                                        <span style="width: 12px; text-align: center; color: var(--gray-400);">=</span>
                                        <span data-worktype-total-for="${workType.id}" style="padding: 1px 4px; border-radius: 3px; color: var(--primary); font-weight: 600; flex: 1; text-align: right;">${UI.formatCurrency(wtTotal, this.currentProject?.currency)}</span>
                                    </div>
                                </div>
                                <div style="display: flex; align-items: center; gap: 8px; flex-shrink: 0;">
                                    ...
                                    <button onclick="EstimateManager.deleteWorkType('${workType.id}'); event.stopPropagation();" class="btn btn-sm" title="Удалить вид работ" style="width: 26px; height: 26px; padding: 0; border-radius: 4px; display: inline-flex; align-items: center; justify-content: center; background: var(--white); border: 1px solid var(--danger); color: var(--danger);" onmouseover="this.style.background='var(--gray-50)'" onmouseout="this.style.background='var(--white)'">×</button>
                                </div>
                            </div>
                        </div>
                        <div id="worktype-${workType.id}-resources" style="display: none; margin-top: 6px;"></div>
                    </div>
                `;
            }
            container.innerHTML = html;
    ...
    async loadResourcesTree(workTypeId) {
        ...
        try {
            ...
            if (filteredResources.length === 0 && isFilterActive) {
                container.innerHTML = `<div style="padding: 6px 8px; color: var(--gray-500); font-size: 12px;">Нет ресурсов по фильтру</div>`;
                return;
            }
            ...
            for (let index = 0; index < filteredResources.length; index++) {
                const r = filteredResources[index];
                ...
                const displayNo = this.escapeHtml(stored || fallbackNo);
                ...
                if (safeType === 'material') {
                    ...
                    // Резервируем место для иконки связи, чтобы кнопки удаления были выровнены
                    linkButtonHtml = `<div style="width: 24px; height: 24px; flex-shrink: 0;"></div>`;
                }
                        ...
                        <div style="display: flex; justify-content: space-between; gap: 10px;">
                            <div style="min-width: 0; flex: 1;">
                                <div style="display: flex; align-items: flex-start; gap: 8px; min-width: 0; width: 100%;">
                                    ...
                                    <div style="min-width: 64px; display: inline-flex; align-items: center; justify-content: center; gap: 6px; flex-shrink: 0; align-self: center;">
                                        <span style="min-width: 36px; text-align: center; font-size: 12px; color: var(--gray-500); flex-shrink: 0; padding: 0 4px;">${displayNo}</span>
                                    ...
                                    <div style="min-width: 0; flex: 1; display: flex; align-items: center; justify-content: space-between;">
                                        <div style="display: flex; align-items: center; gap: 6px; min-width: 0; flex: 1; color: var(--gray-700);">
                                            <span class="resource-cipher-inline" title="Шифр" style="width: 70px; flex-shrink: 0; padding: 1px 4px; background: ${r.cipher ? 'var(--gray-100)' : 'transparent'}; border: 1px solid ${r.cipher ? 'var(--gray-300)' : 'transparent'}; border-radius: 3px; color: var(--gray-700); font-size: 10px; font-weight: 600; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; display: inline-block; text-align: center; box-sizing: border-box;">${r.cipher ? this.escapeHtml(r.cipher) : ''}</span>
                                            <span class="resource-name-inline" data-resource-id="${r.id}" data-worktype-id="${workTypeId}" style="flex: 1; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; cursor: text; padding: 1px 4px; border-radius: 3px; ${metricsStyle}" onmouseover="this.style.background='var(--gray-100)'" onmouseout="this.style.background='transparent'">${this.escapeHtml(r.name)}</span>
                                        </div>
                                        
                                        <div style="display: flex; align-items: center; gap: 4px; flex-shrink: 0; font-size: 12px; color: var(--gray-600); margin-left: auto; width: 460px; justify-content: flex-start;">
                                            <span class="resource-unit-inline" data-resource-id="${r.id}" data-worktype-id="${workTypeId}" style="width: 56px; text-align: left; cursor: text; padding: 1px 4px; border-radius: 3px; ${metricsStyle}" onmouseover="this.style.background='var(--gray-100)'" onmouseout="this.style.background='transparent'">${this.escapeHtml(r.unit ?? '')}</span>
                                            <span class="resource-norm-inline" title="Норма на ед. (Кол-во 1)" style="width: 65px; flex-shrink: 0; padding: 1px 4px; background: ${r.normPerUnit ? '#fffbe6' : 'transparent'}; border: 1px solid ${r.normPerUnit ? '#ffe58f' : 'transparent'}; border-radius: 3px; color: #856404; font-size: 10px; font-weight: 600; display: inline-block; text-align: center; box-sizing: border-box;">${r.normPerUnit ? UI.formatNumber(r.normPerUnit) : ''}</span>
                                            <span class="resource-quantity-inline" data-resource-id="${r.id}" data-worktype-id="${workTypeId}" style="width: 80px; text-align: right; cursor: text; padding: 1px 4px; border-radius: 3px; ${metricsStyle}" onmouseover="this.style.background='var(--gray-100)'" onmouseout="this.style.background='transparent'">${UI.formatNumber(r.quantity ?? 0)}</span>
                                            <span style="width: 12px; text-align: center; color: var(--gray-400);">×</span>
                                            <span class="resource-unitprice-inline" data-resource-id="${r.id}" data-worktype-id="${workTypeId}" style="width: 70px; text-align: right; cursor: text; padding: 1px 4px; border-radius: 3px; ${metricsStyle}" onmouseover="this.style.background='var(--gray-100)'" onmouseout="this.style.background='transparent'">${UI.formatNumber(r.unitPrice ?? 0, 2)}</span>
                                            <span style="width: 12px; text-align: center; color: var(--gray-400);">=</span>
                                            <span style="padding: 1px 4px; border-radius: 3px; ${metricsStyle || 'color: var(--primary);'} font-weight: 600; flex: 1; text-align: right;">${UI.formatCurrency(totalCost, this.currentProject?.currency)}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div style="display: flex; align-items: center; gap: 8px; flex-shrink: 0;">
                                ${linkButtonHtml}
                                <button onclick="EstimateManager.deleteResource('${r.id}'); event.stopPropagation();" class="btn btn-sm" title="Удалить ресурс" style="width: 26px; height: 26px; padding: 0; border-radius: 4px; display: inline-flex; align-items: center; justify-content: center; background: var(--white); border: 1px solid var(--danger); color: var(--danger);" onmouseover="this.style.background='var(--gray-50)'" onmouseout="this.style.background='var(--white)'">×</button>
                            </div>
                        </div>
                    </div>
                `;
        ...
        rootEl.querySelectorAll('.resource-unit-inline[data-resource-id]').forEach((el) => {
            ...
            const bind = (currentValue) => {
                this.makeEditableSelect(el, currentValue, unitOptions, async (newValue) => {
                    ...
                    await api.updateResource(resourceId, updateData);
                    el.textContent = newValue;

                    // Если тип изменился, нужно обновить иконку и цвета
                ...
                html += `
                    <tr>
                        ...
                        <td>
                            ...
                            <button onclick="EstimateManager.deleteStage('${stage.id}')" class="btn" style="padding: 4px 12px; height: auto; background: var(--white); border: 1px solid var(--danger); color: var(--danger);" onmouseover="this.style.background='var(--gray-50)'" onmouseout="this.style.background='var(--white)'">
                                Удалить
                            </button>
                        </td>
                    </tr>
                ...
                html += `
                    <tr>
                        ...
                        <td>
                            ...
                            <button onclick="EstimateManager.deleteWorkType('${workType.id}')" class="btn" style="padding: 4px 12px; height: auto; background: var(--white); border: 1px solid var(--danger); color: var(--danger);" onmouseover="this.style.background='var(--gray-50)'" onmouseout="this.style.background='var(--white)'">
                                Удалить
                            </button>
                        </td>
                    </tr>
                ...
                html += `
                    <tr>
                        ...
                        <td>
                            ...
                            <button onclick="EstimateManager.deleteResource('${resource.id}')" class="btn" style="padding: 4px 12px; height: auto; background: var(--white); border: 1px solid var(--danger); color: var(--danger);" onmouseover="this.style.background='var(--gray-50)'" onmouseout="this.style.background='var(--white)'">
                                Удалить
                            </button>
                        </td>
                    </tr>
