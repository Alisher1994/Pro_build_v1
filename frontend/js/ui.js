...
// Fallback stub for Utils if not loaded (prevents runtime errors in other modules)
if (!window.Utils) {
    window.Utils = {
        formatNumber: (num, fractionDigits) => {
            if (num === null || num === undefined || isNaN(num)) return '0';
            const minFrac = (fractionDigits === undefined || fractionDigits === null) ? 0 : fractionDigits;
            const maxFrac = (fractionDigits === undefined || fractionDigits === null) ? 6 : fractionDigits;
        ...
        formatCurrency: (amount, currencyCode = 'RUB') => {
            ...
            return `${Number(amount || 0).toLocaleString('ru-RU', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${symbol}`;
        },
        ...
        showNotification: (msg) => alert(typeof msg === 'string' ? msg : JSON.stringify(msg)),
        confirm: async () => true,
    };
    console.warn('Utils stubbed (source js/utils.js not loaded)');
}
...
const UI = {
                ...
                <div class="project-modal-left">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="date" id="project-start-date" value="${existingProject?.startDate ? existingProject.startDate.split('T')[0] : ''}">
                        </div>
                        <div class="form-group">
                            <label>Дата окончания</label>
                            <input type="date" id="project-end-date" value="${existingProject?.endDate ? existingProject.endDate.split('T')[0] : ''}">
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-group">
    ...
    showNotification(message, type = 'info') {
        ...
        if (!window.Utils) {
            console.warn('Utils not available to show notification', { message, type });
            alert(msg); // простая подсказка, если Utils не загрузился
            return;
        }
    ...
    showConfirmDialog(title, message, confirmText = 'Да', cancelText = 'Нет') {
        if (!window.Utils) {
            console.warn('Utils not available to show confirm', { title, message });
    ...
    confirmDelete(message, callback) {
        if (!window.Utils) {
            console.warn('Utils not available to confirm delete', { message });
            return;
        }
