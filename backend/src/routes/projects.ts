  ...
  try {
    ...
    const [total, projects] = await Promise.all([
      ...
      prisma.project.findMany({
        ...
        // Делает "Главный офис" (isDeletable=false) первым, затем остальные по дате
        orderBy: [
          { isDeletable: 'asc' },
          { createdAt: 'desc' },
        ],
...
router.post('/', async (req, res) => {
  try {
    const {
      name, description, address, latitude, longitude, photo, genplan, render,
      client, manager, deputy, customer, contractor, currency, startDate, endDate,
      status, coordinates, shiftDuration, workWeek
    } = req.body;
    ...
    const project = await (prisma.project as any).create({
      data: {
        ...
        shiftDuration: shiftDuration !== undefined ? Number(shiftDuration) : 8,
        workWeek: workWeek || '5_2',
      },
    });
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (shiftDuration !== undefined) updateData.shiftDuration = Number(shiftDuration);
    if (workWeek !== undefined) updateData.workWeek = workWeek;
