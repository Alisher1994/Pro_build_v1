...
router.post('/bids/:id/create-contract', async (req: Request, res: Response) => {
  try {
    ...
    const bid = await prisma.tenderBid.findUnique({ where: { id } });
    ...
    const updatedBid = await prisma.tenderBid.update({
      ...
      data: {
        status: 'contract',
        contractNumber: 'Д-' + Math.floor(Math.random() * 10000),
        contractDate: new Date()
      }
    });

    await prisma.tenderInvite.update({
      where: { id: bid.inviteId },
      data: { status: 'contract' }
    });
...
router.post('/auth', async (req: Request, res: Response) => {
  try {
    ...
    // Mark that subcontractor has logged in through the universal link
    await prisma.tenderInvite.update({
      where: { id: invite.id },
      data: { status: 'login' }
    });
