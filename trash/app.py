...
def get_club_settings_instance():
    ...
    if school_id:
        ...
        settings = ClubSettings(
            ...
            notification_hours_before=None
        )
...
def ensure_club_settings_columns():
    ...
    with db.engine.begin() as conn:
        ...
        if 'notification_hours_before' not in columns:
            conn.execute(db.text("ALTER TABLE club_settings ADD COLUMN notification_hours_before INTEGER"))
...
def get_club_settings():
    ...
    try:
        ...
        return jsonify({
            ...
            'notification_hours_before': getattr(settings, 'notification_hours_before', None)
        })
    except Exception as e:
...
def update_club_settings():
    ...
    try:
        ...
        notification_hours_before = data.get('notification_hours_before')
        if notification_hours_before is not None:
            notification_hours_before = int(notification_hours_before) if notification_hours_before else None
            if notification_hours_before is not None and (notification_hours_before < 1 or notification_hours_before > 5):
                return jsonify({'success': False, 'message': 'Время отправки уведомлений должно быть от 1 до 5 часов'}), 400
        ...
        settings.notification_hours_before = notification_hours_before
        db.session.commit()
        return jsonify({'success': True})
    except Exception as e:
        db.session.rollback()
...
def add_group():
    ...
    try:
        ...
        # Проверка уникальности названия группы в рамках школы
        school_id = get_current_school_id()
        if school_id:
            existing_group = Group.query.filter_by(name=name, school_id=school_id).first()
            if existing_group:
                return jsonify({'success': False, 'message': 'Группа с таким названием уже существует в вашей школе'}), 400
        else:
            # Для супер-админа проверяем без school_id (но это редко)
    ...
    try:
        ...
        if 'schedule_time' in data and data['schedule_time']:
            try:
                ...
                return jsonify({'success': False, 'message': f'Неверный формат времени: {data["schedule_time"]}'}), 400
        if 'late_threshold' in data:
            try:
                group.late_threshold = int(data['late_threshold'])
            except (ValueError, TypeError):
                return jsonify({'success': False, 'message': 'Неверное значение порога опоздания'}), 400
        if 'max_students' in data:
            max_students = data['max_students']
            if max_students is None or max_students == '':
                group.max_students = None
            else:
                try:
                    group.max_students = int(max_students)
                except (ValueError, TypeError):
                    return jsonify({'success': False, 'message': 'Неверное значение максимального количества учеников'}), 400
        if 'notes' in data:
...
def get_cash_balance():
    ...
    # Общий приход (фильтруем по школе через Student, только наличные платежи (cash), только положительные суммы и не NULL)
    # Учитываем только payment_type = 'cash' или NULL (старые записи считаем наличными)
    if is_super_admin():
        total_income = db.session.query(func.sum(Payment.amount_paid)).filter(
            Payment.amount_paid.isnot(None),
            Payment.amount_paid > 0,
            or_(Payment.payment_type == 'cash', Payment.payment_type.is_(None))  # Только наличные или NULL (старые записи)
        ).scalar() or 0
    else:
        if school_id:
            income_query = db.session.query(Payment).join(
    ...
    # Общий расход (фильтруем по школе, только положительные суммы и не NULL)
    if is_super_admin():
        ...
        if school_id:
            expense_query = Expense.query.filter_by(school_id=school_id).filter(
                Expense.amount.isnot(None),
                Expense.amount > 0
            )
    ...
    # Общая сумма переданных средств (фильтруем по школе, только положительные суммы и не NULL)
    if is_super_admin():
        total_transferred = db.session.query(func.sum(CashTransfer.amount)).filter(
            CashTransfer.amount.isnot(None),
            CashTransfer.amount > 0
        ).scalar() or 0
    else:
        if school_id:
            transfer_query = CashTransfer.query.filter_by(school_id=school_id).filter(
                CashTransfer.amount.isnot(None),
                CashTransfer.amount > 0
            )
    ...
    # Остаток
    balance = float(total_income) - float(total_expenses) - float(total_transferred)
...
def transfer_payment():
    """Перевести оплаченную сумму от одного ученика к другому"""
    if getattr(current_user, 'role', None) not in ['admin', 'financier', 'payment_admin']:
        return jsonify({'success': False, 'message': 'Нет доступа'}), 403
    ...
    try:
        ...
        transfer_amount = float(data.get('amount', 0))
        notes = data.get('notes', '').strip()
        
        if not payment_id or not from_student_id or not to_student_id:
            return jsonify({'success': False, 'message': 'Не указаны обязательные параметры'}), 400
        
        if transfer_amount <= 0:
            return jsonify({'success': False, 'message': 'Сумма перевода должна быть больше нуля'}), 400
        
        # Получить оплату с проверкой school_id
        payment_query = Payment.query.filter_by(id=payment_id)
        payment = filter_query_by_school(payment_query, Payment).first()
        if not payment:
            return jsonify({'success': False, 'message': 'Оплата не найдена'}), 404
        
        # Проверить, что оплата принадлежит указанному ученику
        if payment.student_id != from_student_id:
            return jsonify({'success': False, 'message': 'Оплата не принадлежит указанному ученику'}), 400
        
        # Проверить, что сумма перевода не превышает сумму оплаты
        if transfer_amount > payment.amount_paid:
            return jsonify({'success': False, 'message': f'Сумма перевода не может превышать сумму оплаты ({payment.amount_paid:.2f} сум)'}), 400
        
        # Получить учеников с проверкой school_id
        from_student_query = Student.query.filter_by(id=from_student_id)
        from_student = filter_query_by_school(from_student_query, Student).first()
        if not from_student:
            return jsonify({'success': False, 'message': 'Ученик-отправитель не найден'}), 404
        
        to_student_query = Student.query.filter_by(id=to_student_id)
        to_student = filter_query_by_school(to_student_query, Student).first()
        if not to_student:
            return jsonify({'success': False, 'message': 'Ученик-получатель не найден'}), 404
        
        # Проверить, что оба ученика из одной школы
        if from_student.school_id != to_student.school_id:
            return jsonify({'success': False, 'message': 'Нельзя переводить средства между учениками разных школ'}), 400
        
        # Уменьшить сумму оплаты ученика А
        payment.amount_paid = payment.amount_paid - transfer_amount
        
        # Если сумма стала 0 или отрицательной, можно удалить оплату или оставить с нулевой суммой
        # Оставим с нулевой суммой для истории
        if payment.amount_paid < 0:
            return jsonify({'success': False, 'message': 'Сумма перевода превышает доступную сумму оплаты'}), 400
        
        # Обновить примечание к исходной оплате
        transfer_note = f"Переведено {transfer_amount:.2f} сум ученику {to_student.full_name}"
        if notes:
            transfer_note += f". {notes}"
        if payment.notes:
            payment.notes = f"{payment.notes}\n{transfer_note}"
        else:
            payment.notes = transfer_note
        
        # Создать новую оплату для ученика Б
        # Используем те же параметры, что и у исходной оплаты (месяц, год, тариф)
        new_payment = Payment(
            ...
            notes=f"Перевод от {from_student.full_name}" + (f". {notes}" if notes else ""),
            created_by=current_user.id,
        ...
        return jsonify({
            'success': True,
            'message': f'Сумма {transfer_amount:.2f} сум успешно переведена от {from_student.full_name} к {to_student.full_name}',
            'from_student_balance': calculate_student_balance(from_student),
            'to_student_balance': calculate_student_balance(to_student)
        })
    except Exception as e:
        db.session.rollback()
        import traceback
        print(f"Ошибка при переводе оплаты: {traceback.format_exc()}")
        return jsonify({'success': False, 'message': str(e)}), 500
...
def send_automatic_group_notifications_job():
    """Задача для планировщика - автоматическая отправка уведомлений о занятиях"""
    with app.app_context():
        try:
            from backend.services.telegram_service import send_automatic_group_notifications
            result = send_automatic_group_notifications()
            if result.get('sent_count', 0) > 0:
                print(f"[INFO] Автоматические уведомления: отправлено {result.get('sent_count')} уведомлений")
            if result.get('failed_count', 0) > 0:
                print(f"[WARNING] Автоматические уведомления: не отправлено {result.get('failed_count')} уведомлений")
        except Exception as e:
            import traceback
            print(f"[ERROR] Ошибка при автоматической отправке уведомлений: {e}")
...
def setup_scheduler():
    ...
    # Запускать каждый час для проверки и отправки уведомлений о занятиях
    scheduler.add_job(
        func=send_automatic_group_notifications_job,
        trigger=CronTrigger(minute=0),  # Каждый час в 0 минут
        id='send_group_notifications',
        name='Автоматическая отправка уведомлений о занятиях',
        replace_existing=True
    )
    
    scheduler.start()
    print("[OK] Планировщик запущен: автоматическая отправка напоминаний об оплате и уведомлений о занятиях")
    return scheduler
